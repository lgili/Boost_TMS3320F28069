...
*  local variables
***********************************/
unsigned long ui32Base = 0x00007050;   // SCI-A
uint32_t vaca =0;
...
void FMSTR_ProcessSCI(void)
{
    ...
    if (pcm_wFlags.flg.bTxActive)
    {
        ...
        // TODO pegar espaço do TX
        /*if(get_tx_free_space())
        {
            FMSTR_U8 ch;
            // just put the byte into the SCI transmit buffer //
            if(!FMSTR_Tx(&ch))
                put_byte_in_tx_buffer((unsigned char) ch);
        ...
        if(SciaRegs.SCIFFTX.bit.TXFFST != 0)
        {
            FMSTR_U8 ch;
            if(!FMSTR_Tx(&ch))
                scia_xmit((FMSTR_U8) ch);
                //UARTCharPut(ui32Base,(FMSTR_U8) ch);
        }

#if FMSTR_SCI_HAS_TXQUEUE
        /* waiting for transmission complete flag? */
        //if(pcm_wFlags.flg.bTxWaitTC && FMSTR_SCI_TXEMPTY(wSciSR))
        // TODO verificar se ta vazio o TX
        if(pcm_wFlags.flg.bTxWaitTC && UARTSpaceAvail(ui32Base))
        {
        ...
        {
        volatile unsigned char *rx_byte2;
        // TODO checar se tem algo e se tiver pegar o byte

        //if (get_uart_rx_byte(&rx_byte2))
        //if(SciaRegs.SCIFFTX.bit.TXFFST != 0) // retorna -1 se não tem nada na FIFO
       // {
            uart_receive(&rx_byte2);
            //rx_byte2 = (unsigned char)UARTCharGetNonBlocking(ui32Base);
            /*lint -esym(550, nRxChar) */
            //volatile FMSTR_U16 nRxChar;
            //nRxChar = FMSTR_SCI_GETCHAR();
            FMSTR_UNUSED((FMSTR_U16)rx_byte2);
       // }
        }
#endif
    }
    ...
    {
        /* data byte received? */
        volatile unsigned char *rx_byte = 0U;
        // TODO checar se tem algo e se tiver pegar o byte
        //rx_byte = UARTCharGetNonBlocking(ui32Base);
        //if (get_uart_rx_byte(&rx_byte2))
        //if(UARTCharsAvail(ui32Base)) // retorna -1 se não tem nada na FIFO
        //if (FMSTR_SCI_RXREADY(wSciSR))
        //{
            uart_receive(&rx_byte);
            //rx_byte = (unsigned char)UARTCharGetNonBlocking(ui32Base);
            FMSTR_BCHR nRxChar = 0U;
            nRxChar = (FMSTR_BCHR) rx_byte;
...
void FMSTR_SendResponse(FMSTR_BPTR pResponse, FMSTR_SIZE8 nLength)
{
...
#if (FMSTR_USE_SCI) || (FMSTR_USE_ESCI) || (FMSTR_USE_LPUART)
    {
        ...
        // TODO coloca byte no TX
        //put_byte_in_tx_buffer(FMSTR_SOB);
        //UARTCharPut(ui32Base, FMSTR_SOB);
        scia_xmit(FMSTR_SOB);
    }
